...
int main() {
    ...
    while (window.isOpen()) {
        ...
        // update particles 
        std::vector<std::thread> particle_thread = update_particles(particles);

        // Update sprite in another thread 
        std::thread sprite_thread = spriteManager.updateAsync(window, isExplorerMode);
        ...
        for (auto& thread : particle_thread) {
            thread.join(); 
        }
